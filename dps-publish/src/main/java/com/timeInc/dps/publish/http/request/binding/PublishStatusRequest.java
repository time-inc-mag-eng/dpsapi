...
/**
 * The http metadata associated with get publishing status request/response
 * see Adobe Publishing API documentation for more information.
 */
public class PublishStatusRequest extends AbstractPublishRequest<PublishingStatus> {
	private final TicketConfig config;
	
	
	/**
	 * Instantiates a new publish status request.
	 *
	 * @param basePath the base path
	 * @param config the config
	 */
	public PublishStatusRequest(String basePath, TicketConfig config) {
		super(basePath);
		this.config = config;
	}
	...
	protected String getRelativePath() {
		return "publishRequests/" + config.getRequestId();
	}
}
